...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <nav className={`w-full z-50 transition-all duration-300 ${isHome ? 'absolute top-0 left-0 bg-transparent py-4' : 'sticky top-0 bg-white/95 backdrop-blur-md shadow-sm border-b border-gray-100'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <Link to={isAuthority ? '/dashboard' : '/'} className="flex items-center group">
            ...
            {/* Mobile Top Actions (User button / Login) */}
      ...
      {/* Mobile Bottom Navigation Bar — hidden for authority users */}
      {!isAuthority && (
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-[99] pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          <div className="flex justify-around items-center h-16 px-2 relative">
            ...
            <Link to={isSignedIn ? "/profile" : "/login"} className={`flex flex-col items-center justify-center w-full h-full gap-1 ${isActive('/profile') ? 'text-primary' : 'text-gray-500 active:text-gray-900'}`}>
               <User className={`w-6 h-6 ${isActive('/profile') ? 'fill-primary/20' : ''}`} />
