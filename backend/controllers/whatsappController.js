  ...
  try {
    ...
    if (msg.type === "text") {
      ...
      await sendWhatsAppMessage(
        phone,
        "Sorry, only text, image, and location are supported.",
      );
    }
  } catch (err) {
...
async function handleTextMessage(msg, session) {
  ...
  if (session.step === "INIT") {
    if (text.toLowerCase() === "report") {
      ...
      console.log(
        "About to send WhatsApp message to",
        phone,
        "with text:",
        "What is the issue category? (e.g., Roads, Water, Garbage, Electricity, Waste Management, Public Safety, Other Issues)",
      );
      await sendWhatsAppMessage(
        phone,
        "What is the issue category? (e.g., Roads, Water, Garbage, Electricity, Waste Management, Public Safety, Other Issues)",
      ...
      console.log(
        ...
        phone,
        "with text:",
        'Send "report" to start reporting a civic issue.',
      );
      await sendWhatsAppMessage(
        phone,
        'Send "report" to start reporting a civic issue.',
    ...
    console.log(
      ...
      phone,
      "with text:",
      "Please send an image of the issue.",
    );
    ...
    const issue = new Issue({
      ...
      waImageUrl: session.data.waImageUrl || session.data.imageUrl,
      imageUrl: session.data.waImageUrl || session.data.imageUrl,
    ...
    console.log(
      "handleTextMessage: Unexpected step, sending fallback message.",
    );
    console.log(
      "About to send WhatsApp message to",
      phone,
  ...
  if (session.step === "WAITING_FOR_IMAGE") {
    ...
    if (!mediaId) {
      console.log(
        ...
        phone,
        "with text:",
        "Could not get image. Please resend.",
      );
    ...
    session.data.waImageUrl = storedUrl;
    // Try Gemini auto-detect category from image
    const geminiCategory = await detectCategoryWithGemini({
      imageUrl: storedUrl,
    });
    if (geminiCategory) {
    ...
    console.log(
      "handleImageMessage: Unexpected step, sending fallback message.",
    );
    console.log(
      "About to send WhatsApp message to",
      phone,
  ...
  if (session.step === "WAITING_FOR_LOCATION") {
    ...
    if (lat == null || lng == null) {
      console.log(
        ...
        phone,
        "with text:",
        "Could not get location. Please resend.",
      );
      await sendWhatsAppMessage(
        phone,
        "Could not get location. Please resend.",
    ...
    console.log(
      "handleLocationMessage: Unexpected step, sending fallback message.",
    );
    console.log(
      "About to send WhatsApp message to",
      phone,
